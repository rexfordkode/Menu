package com.example.moftech.smartmenu.Database;
...
public class Database extends SQLiteAssetHelper {
    private static final String DB_NAME ="smenu.db";
    ...
    public List<Order> getCarts(String userPhone)
    {
        ...
        Cursor curs = queryBuild.query(db,sqlSelect,"UserPhone=?",new String[]{userPhone},null,null,null);

        final List<Order> result = new ArrayList<>();
        if (curs.moveToFirst()){
            do {
                result.add(new Order(
                        curs.getString(curs.getColumnIndex("UserPhone")),
                        curs.getString(curs.getColumnIndex("ProductId")),
                        curs.getString(curs.getColumnIndex("ProductName")),
                        curs.getString(curs.getColumnIndex("Quantity")),
                        curs.getString(curs.getColumnIndex("Price")) ,
                        curs.getString(curs.getColumnIndex("Discount")),
                        curs.getString(curs.getColumnIndex("Image"))
                ));
    ...
    {
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("INSERT INTO Favorites (FoodId,UserPhone,FoodName,FoodPrice,FoodMenuId,FoodImage,FoodDiscount,FoodDescription) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s');",
                food.getFoodId(),
                food.getUserPhone(),
                food.getFoodName(),
                food.getFoodPrice(),
        ...
        if (curs.moveToFirst()){
            do {
                result.add(new Favorites(
                        curs.getString(curs.getColumnIndex("FoodId")),
                        curs.getString(curs.getColumnIndex("UserPhone")),
                        curs.getString(curs.getColumnIndex("FoodName")),
                        curs.getString(curs.getColumnIndex("FoodPrice")),
                        curs.getString(curs.getColumnIndex("FoodMenuId")) ,
                        curs.getString(curs.getColumnIndex("FoodImage")),
                        curs.getString(curs.getColumnIndex("FoodDiscount")),
                        curs.getString(curs.getColumnIndex("FoodDescription"))
